...
func (r *UpdatePushEventRequest) validate() error {
	if r == nil {
		return errors.New("request is nil")
	}

	if r.Event.Status != 0 {
		if r.Event.Status < EventStatusPending || r.Event.Status > EventStatusWhitelisted {
			return errors.New("invalid event.status")
		}
	}

	if r.Event.PushLevel != "" {
		switch r.Event.PushLevel {
		case PushLevelFatal, PushLevelWarning, PushLevelReminder:
		default:
			return errors.New("invalid push_level")
		}
	}
