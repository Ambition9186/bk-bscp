...
type TemplateSpace interface {
	...
	GetBizTemplateSpaceByName(kit *kit.Kit, bizID, projectID uint32, name string) (*table.TemplateSpace, error)
...
// GetBizTemplateSpaceByName Get template space by name.
func (dao *templateSpaceDao) GetBizTemplateSpaceByName(kit *kit.Kit, bizID, projectID uint32, name string) (
	*table.TemplateSpace, error) {
	m := dao.genQ.TemplateSpace

	return dao.genQ.TemplateSpace.WithContext(kit.Ctx).Where(m.BizID.Eq(bizID), m.ProjectID.Eq(projectID), m.Name.Eq(name)).Take()
}
