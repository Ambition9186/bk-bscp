...
func (s *Service) UpsertTableContent(ctx context.Context, req *pbds.UpsertTableContentReq) (
	*pbds.UpsertTableContentResp, error) {
	...
	// 1. 查询出主键，按主键来对比
	for _, v := range tableStruct.Spec.Columns_ {
		if v.Primary && v.ColumnType != table.NumberColumn {
			return nil, fmt.Errorf("the primary key must be a number type")
		}
		if v.Primary {
			primary = v.Name
		}
	...
	existingDatas := make(map[float64]string, 0)
	existingIDs := map[float64]uint32{}
	existingStates := make(map[float64]string, 0)
	if count > 0 {
		for _, v := range contents {
			value := v.Spec.Content[primary].(json.Number)
			...
			existingDatas[numValue] = string(data)
			existingIDs[numValue] = v.ID
			existingStates[numValue] = v.Spec.Status
		}
	}

	// 处理提交的数据
	submitDatas := make(map[float64]string, 0)
	for _, v := range req.GetContents() {
		...
		numValue := v.AsMap()[primary].(float64)
		submitDatas[numValue] = string(jsonData)
	}
...
func getDifference(array1, array2 map[float64]string) []float64 {
	var diff []float64
